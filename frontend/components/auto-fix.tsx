...
export function AutoFix({ file, report }: { file: File; report: AnalysisReport }) {
  ...
  return (
    <section className="rounded-[1.6rem] border border-signal/25 bg-signal/[.07] p-4 sm:rounded-[2rem] sm:p-8">
      ...
      <div className="mt-5 rounded-2xl border border-signal/25 bg-white/70 p-4 dark:bg-white/[.06] sm:mt-6">
      ...
      <div className="mt-6 grid gap-3 lg:grid-cols-3">
        {options.map((option) => <button key={option.id} type="button" disabled={generating} onClick={() => { setStrategy(option.id); setDownloadUrl(""); }} className={`lift-card min-h-32 rounded-2xl border p-4 text-left ${strategy === option.id ? "border-signal bg-white shadow-sm dark:bg-white/10" : "border-black/10 hover:border-signal/50 dark:border-white/10"}`}><span className="text-xs font-bold uppercase tracking-wider text-signal">{option.id === recommendation.strategy ? "Recommended" : option.note}</span><strong className="mt-2 block">{option.title}</strong><span className="mt-2 block text-xs leading-5 opacity-55">{option.copy}</span></button>)}
      </div>
      {generating && <div className="mt-6" aria-live="polite"><div className="mb-2 flex flex-col items-stretch justify-between gap-3 text-sm font-semibold sm:flex-row sm:items-center sm:gap-4"><span>Generating safer video · {progress}%</span><button type="button" onClick={cancel} className="rounded-full border border-black/15 px-4 py-2.5 text-xs font-bold hover:border-red-400 hover:text-red-600 dark:border-white/20 sm:py-2">Cancel generation</button></div><div className="h-2 overflow-hidden rounded-full bg-black/10 dark:bg-white/10"><div className="h-full rounded-full bg-[#19a974] transition-all" style={{ width: `${progress}%` }} /></div></div>}
      ...
      <div className="mt-6 flex flex-col items-stretch gap-3 sm:flex-row sm:flex-wrap sm:items-center">
        {downloadUrl ? <a className="rounded-full bg-signal px-6 py-3.5 text-center text-sm font-bold text-white hover:bg-[#148b60] sm:py-3" href={downloadUrl} download={`${file.name.replace(/\.[^.]+$/, "")}-${strategy}-safer.${format}`}>Download {format.toUpperCase()}</a> : <button type="button" disabled={generating} onClick={generate} className="rounded-full bg-signal px-6 py-3.5 text-sm font-bold text-white hover:bg-[#148b60] disabled:opacity-50 sm:py-3">Generate safer version</button>}
